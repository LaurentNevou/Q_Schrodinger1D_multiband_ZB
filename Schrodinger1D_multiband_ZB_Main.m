...
PrintResultsISB = 1;            % Switch to print or not the ISB dipoles on the shell
PlotMass        = 0;            % PLot meff in the Kane and Luttinger model
...
meK2  = 1 ./ ( 1 +  EPmat ./ (EEmat + Egmat-V0mat+1/3*Dsomat)  ); % m(z,E) in the 2 bands Kane s model
...
E=[];
j=0;FS=15;
s{1}=strcat('\fontsize{',num2str(FS),'}\color{blue}Potential');
s{2}=strcat('\fontsize{',num2str(FS),'}\color{blue}----------');
...
if FDM_kp1bandCB==1
    ...
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> FDM kp1bandCB: m=m(z)');
...
if FDM_kp1bandHH==1
    ...
    E{j}=-E{j};
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> FDM kp1bandHH: m=m(z)');
...
if FDM_kp1bandLH==1
    ...
    E{j}=-E{j};
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> FDM kp1bandLH: m=m(z)');
...
if FDM_kp2bands_Kane==1
    ...
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> FDM kp2bands Kane: m=m(z,E)');
...
if FDM_kp3bands_Kane==1
    ...
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> FDM kp3bands Kane: m=m(z,E)');
...
if FDM_kp6bands_Luttinger==1
    ...
    E{j}=-E{j};
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> HH 1band: m=m(z)');
    j=j+1;
    [E{j},psi{j}] = Schrod_2bands_Luttinger_Kohn_f(z,V0,Eg,Dso,g1,g2,g3,n,av,bv,exx,ezz);
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> FDM kp6bands Luttinger: m=m(z,E)');
...
if ShootingCB_1band_Kane==1
    ...
    j=j+1;
    Mass=meK2(1,:);  % m=m(z)
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> ShootingCB 1band Kane: m=m(z)');
...
if ShootingHH_1band==1
    ...
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> ShootingHH 1band: m=m(z)');
...
if ShootingCB_2bands_Kane==1
    ...
    j=j+1;
    Mass=meK2; % m=m(z,E)
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> ShootingCB 2bands Kane: m=m(z,E)');
...
if ShootingCB_3bands_Kane==1
    ...
    j=j+1;
    Mass=meK3; % m=m(z,E)
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> ShootingCB 3bands Kane: m=m(z,E)');
    [E{j},psi{j}] = Schrod_Nbands_shoot_f(z,V0+DCBO,Mass,n,EE,dE,precision);
...
if ShootingCB_Luttinger==1
    ...
    j=j+1;
    Mass=meL; % m=m(z,E)
    s{j+2}=strcat('\fontsize{',num2str(FS),'}\color[rgb]{',num2str(col(j,:)),'}-> ShootingCB Luttinger: m=m(z,E)');
    [E{j},psi{j}] = Schrod_Nbands_shoot_f(z,V0+DCBO,Mass,n,EE,dE,precision);
...
for j=1:length(E)
  for i=1:length(E{j})
      ...
      plot(z*1e9,PSI{j}(:,i)+shift,'color',col(j,:),'linewidth',2)
